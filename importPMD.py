...
fpath = "/home/halamix2/repos/stunt_gp/stunt_gp_formats/meshdata/car4.PMD"
...
mpath = "/home/halamix2/repos/stunt_gp/sgpToolset/png/"
...
try:
    ...
    def addMesh(name, verts, faces, uvs, materialIndexes, materials, transform):
        ...
        for i in range(len(mesh.polygons)):
            mesh.uv_textures.active.data[i].image = materials[materialIndexes[i]][
                1
            ].texture.image
...
def loadPMD(fpath, mpath, trkset, location=None):
    ...
    # deepcode ignore MissingCloseOnSomePath: this file will be removed and is used for comparison reasons until new importer works as expected
    f = open(fpath, "rb")
    buffer = f.read()
    ...
    sizes = struct = Struct("<34i").unpack_from(
        buffer[sizesTableOffset:]
    )  # 34 int32_t?
    # print(sizes)
    fcnt = int(sizes[5] / 2)
    vcnt = int(sizes[4] / (4 * 4))  # vertex count?
    ...
    for i in range(scnt):
        if ver == 83:
            struct = Struct("<8h3if4h4i").unpack_from(buffer[buffptr:])
            buffptr += 64
    ...
    for surface in surfaces:
        ...
        # uvs_start + uvs_count
        # polys_start + polys_count
        vs = verts[
            surface[4] : surface[4] + surface[2]
        ]  # vertexIndexPtr starts from 0 again
        uvss = uvs[surface[3] : surface[3] + surface[1]]
        ps = polys[surface[5] : surface[5] + surface[0]]
        transform = transforms[surface[6]]  # 0x8
        # polygons
        faces = []
        us = []
        materialIndexes = []
        ...
        ob = addMesh(
            mname + str(loaded), vs, faces, us, materialIndexes, materials, transform
        )
        if location:
